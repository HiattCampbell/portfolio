...
export default function Project(props) {
  ...
  if (
    !Array.isArray(props.project.pictures) ||
    props.project.pictures.length <= 0
  ) {
    return null;
  }
          ...
          {props.project.pictures.map((picture, index) => {
            return (
              <div
                ...
                id="slideImg"
              >
                {(index === current && picture.includes("png") && (
                  <img
                    className="sliderImg"
                    style={{ width: "600px" }}
                    src={picture}
          ...
          {props.project.link == false ? (
            <form action={props.project.repository}>
              <input
                ...
                type="submit"
                value="Visit repository"
              />
            </form>
          ) : (
            <form action={props.project.link}>
              <input className="repoButton" type="submit" value="Visit site" />
            </form>
          )}
        </div>
      </div>
      {props.project.index != 2 ? (
        <hr className="portfolioHr"></hr>
      ) : (
        <div></div>
      )}
    </div>
